...
while True:
    ...
    if opcao.upper() == "D":
        ...
        if valor_deposito > 0:
            ...
            extratos_bancarios["Extrato de Deposito"].append(
                f"Deposito: R$ {valor_deposito:.2f}"
            )

            extratos_bancarios["Extrato Geral"].append(
                f" Deposito: R$ {valor_deposito:.2f}"
            )
        ...
        if numeros_saques < LIMITE_SAQUES:
            ...
            if valor_saque > 0:
                if valor_saque <= limite and valor_saque <= saldo:
                    extratos_bancarios["Extrato de Saques"].append(
                        f"Saque: R$ {valor_saque:.2f}"
                    )
            ...
            print(
                "Você não pode efetuar o saque, você excedeu o limite diário (3 saques)"
            )

    elif opcao.upper() == "E":
        while True:
            opcao_de_estrato = int(
                input(
                    """
